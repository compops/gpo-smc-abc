...
class ssm(object):
    ...
    def generateObservation(self, xt, tt):
        (w, v1, v2) = np.exp(0.5 * xt) * \
            rstable1(self.par[3], 0.0, 1.0, 0, (1, len(xt)))
        ...
        if (self.version == "standard"):
            for v1 in range(0, self.nParInference):
                if v1 == 0:
                    ...
                    # With Atan-transform
                    gradient[:, v1] = 1.0 * sigma * (np.cos(pu * (self.par[3] - 1)) / pw)**(-1 + 1 / self.par[3]) * (self.y[tt] - np.arctan(sigma * (np.cos(pu * (self.par[3] - 1)) / pw)**(-1 + 1 / self.par[3]) * np.sin(self.par[3] * pu) * np.cos(pu)**(-1 / self.par[3]))) * (self.par[3]**2 * pu * np.cos(pu) - self.par[3] * pu * np.sin(self.par[3] * pu) * np.sin(pu * (self.par[3] - 1)) - np.log(np.cos(pu * (self.par[
                        3] - 1)) / pw) * np.sin(self.par[3] * pu) * np.cos(pu * (self.par[3] - 1)) + np.log(np.cos(pu)) * np.sin(self.par[3] * pu) * np.cos(pu * (self.par[3] - 1))) * np.cos(pu)**(1 / self.par[3]) / (self.par[3]**2 * self.epsilon**2 * (sigma**2 * (np.cos(pu * (self.par[3] - 1)) / pw)**(-2 + 2 / self.par[3]) * np.sin(self.par[3] * pu)**2 + np.cos(pu)**(2 / self.par[3])) * np.cos(pu * (self.par[3] - 1)))
        ...
        if (self.version == "tanhexp"):
            for v1 in range(0, self.nParInference):
                if v1 == 0:
                    ...
                    gradient[:, v1] = (xt - self.par[0]) * \
                        Q2 * px * (1.0 - self.par[1]**2)
                elif v1 == 2:
                    ...
                    gradient[:, v1] = -1.0 * sigma * (np.cos(pu * th3) / pw)**(th3 / (th3 + 1.0)) * (yt - np.arctan(sigma * (np.cos(pu * th3) / pw)**(-th3 / (th3 + 1.0)) * np.sin(pu * (th3 + 1.0)) * np.cos(pu)**(-1 / (th3 + 1.0)))) * (pu * (th3 + 1.0)**2 * (th3**2 - 1) * np.cos(pu * (th3 + 1.0)) * np.cos(pu * th3) + (pu * (th3 + 1.0) * (th3**2 - 1) * np.sin(pu * th3) * th3 - (
                        1.0 * th3**2 - 1.0) * np.log(np.cos(pu * th3) / pw) * np.cos(pu * th3)) * np.sin(pu * (th3 + 1.0)) + (th3**2 - 1) * np.log(np.cos(pu)) * np.sin(pu * (th3 + 1.0)) * np.cos(pu * th3)) * np.cos(pu)**(1 / (th3 + 1.0)) / (eps**2 * (sigma**2 * np.sin(pu * (th3 + 1.0))**2 + (np.cos(pu * th3) / pw)**(2 * th3 / (th3 + 1.0)) * np.cos(pu)**(2 / (th3 + 1.0))) * (th3 + 1.0)**2 * np.cos(pu * th3))
                else:
                    gradient[:, v1] = 0.0
   ...
   #=========================================================================
    ...
    def makeNoisy(self, sm):
        if (self.transformY == "arctan"):
            if sm.weightdist == "boxcar":
                self.y = np.arctan(
                    self.ynoiseless) + np.random.uniform(-sm.tolLevel, sm.tolLevel, (self.T, 1))
            elif sm.weightdist == "gaussian":
                self.y = np.arctan(self.ynoiseless) + \
                    sm.tolLevel * np.random.randn(self.T, 1)
        elif (self.transformY == "none"):
            if sm.weightdist == "boxcar":
                self.y = self.ynoiseless + \
                    np.random.uniform(-sm.tolLevel, sm.tolLevel, (self.T, 1))
            elif sm.weightdist == "gaussian":
                self.y = self.ynoiseless + sm.tolLevel * \
                    np.random.randn(self.T, 1)
        else:
            raise NameError("makeNoisy: unknown transformation of data.")
    ...
    def copyData(self, sys):
        ...
        # Check if nQInference and nParInference are set and use default
        # otherwise
        if (self.nQInference == None):
            self.nQInference = np.int(0)
            print(
                "model: assuming that Q-function should not be estimated for this model.")

        if (self.nParInference == None):
            self.nParInference = np.int(2)
            print("model: assuming that " + str(self.nParInference) +
