...
def template_makeNoisy(model, sm):
    if (model.transformY == "arctan"):
        if sm.weightdist == "boxcar":
            model.y = np.arctan(
                model.ynoiseless) + np.random.uniform(-sm.tolLevel, sm.tolLevel, (model.T, 1))
        elif sm.weightdist == "gaussian":
            model.y = np.arctan(model.ynoiseless) + \
                sm.tolLevel * np.random.randn(model.T, 1)
    elif (model.transformY == "none"):
        if sm.weightdist == "boxcar":
            model.y = model.ynoiseless + \
                np.random.uniform(-sm.tolLevel, sm.tolLevel, (model.T, 1))
        elif sm.weightdist == "gaussian":
            model.y = model.ynoiseless + sm.tolLevel * \
                np.random.randn(model.T, 1)
    else:
...
def template_copyData(model, sys):
    ...
    if (model.nParInference == None):
        ...
        print("model: assuming that " + str(model.nParInference) +
...
def empty_Qfunc(model, xtt, xt, st, at, tt):
    raise NameError(
        "No Q-function calculated for this model, cannot use the EM algorithm.")
...
def template_generateData(model, u=None, fileName=None, order=None):
    ...
    if (fileName == None):
        ...
        if (order == None):
            ...
            model.x = np.array(tmp[0:model.T, 0],
                               copy=True).reshape((model.T, 1))
            model.y = np.array(tmp[0:model.T, 1],
                               copy=True).reshape((model.T, 1))
            model.u = u
        elif (order == "xuy"):
            model.x = np.array(tmp[0:model.T, 0],
                               copy=True).reshape((model.T, 1))
            model.u = np.array(tmp[0:model.T, 1],
                               copy=True).reshape((model.T, 1))
            model.y = np.array(tmp[0:model.T, 2],
                               copy=True).reshape((model.T, 1))
        else:
            raise NameError(
