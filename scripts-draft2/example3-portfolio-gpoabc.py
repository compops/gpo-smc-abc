...
# Compute the VAR
correlation, value_at_risk = computeValueAtRisk(
    log_volatility, log_returns[:, 0:nAssets], 0.01)

plt.plot(np.mean(-value_at_risk, axis=1))
plt.plot(np.mean(log_returns[:, 0:nAssets], axis=1), 'k.')
plt.plot(np.mean(value_at_risk, axis=1))

ab1 = np.mean(np.mean(-value_at_risk, axis=1) <
              np.mean(log_returns[:, 0:nAssets], axis=1))
ab2 = np.mean(np.mean(value_at_risk, axis=1) >
              np.mean(log_returns[:, 0:nAssets], axis=1))
