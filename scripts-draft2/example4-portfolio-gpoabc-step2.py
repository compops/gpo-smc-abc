...
def computeValueAtRisk(x, d, Test, alpha):
    ...
    res = optimize.fmin_l_bfgs_b(
        th.evaluateLogPosteriorBFGS, 1.0, approx_grad=1, bounds=b)
...
for ii in range(nAssets):
    models[:, ii] = np.matrix(pd.read_csv(
        'results/example4-portfolio-gpoabc-step1-models' + str(ii // 5) + '.csv'))[:, ii + 1].reshape(4)
    log_volatility[:, ii] = estimateLogVolatility(
        log_returns[0:T, ii], models[:, ii])

# Compute the VAR
dof, correlation, value_at_risk = computeValueAtRisk(
    log_volatility, log_returns[:, 0:nAssets], Test, 0.01)

# Plot
plt.plot(np.mean(value_at_risk[10:], axis=1))
plt.plot(np.mean(log_returns[10:, 0:nAssets], axis=1), 'k.')

# Count number of violations on validation data
np.sum(np.mean(value_at_risk[Test:], axis=1) >
       np.mean(log_returns[Test:, 0:nAssets], axis=1))

# Export
fileOut = pd.DataFrame(value_at_risk)
fileOut.to_csv('results/example4-portfolio-gpoabc-var.csv')
