...
###################################################################################
#
# Makes plot from the run of the files example4-gposmc.py and example4-gpoabc.py
...
VaRABC <- read.table("../results/example4-gpoabc-var.csv",header=T, sep=",", stringsAsFactors=F)
VaRSMC <- read.table("../results/example4-gposmc-var.csv",header=T, sep=",", stringsAsFactors=F)

# Load log-returns
y <- read.table("../results/example4-gpoabc-returns.csv", header=T, sep=",")
...
plot(grid, rowMeans(y[,-1]), pch=19, col="darkgrey", bty="n", ylab="log-returns", xlab="time", 
     cex=0.5, ylim=c(-50,30), xaxt="n", cex.lab=0.75, cex.axis=0.75)
