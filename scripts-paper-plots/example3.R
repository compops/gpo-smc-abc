...
###################################################################################
#
# Makes plots from the run of the files example3-gposmc.py and example3-gpoabc.py
...
VaRABC <- read.table("../results/example3-gpoabc-var.csv",header=T, sep=",", stringsAsFactors=F)
VaRSMC <- read.table("../results/example3-gposmc-var.csv",header=T, sep=",", stringsAsFactors=F)

# Load state (log-volatility) estimates
xhat <- read.table("../results/example3-gpoabc-volatility.csv", header=T, sep=",")

# Load log-returns
y <- read.table("../results/example3-gpoabc-returns.csv", header=T, sep=",")

# Load estimates of model parameters
m <- read.table("../results/example3-gpoabc-model.csv", header=T, sep=",")
...
for (ii in 1:nAssets) {
  for (tt in 1:T) {
      CI[ii,tt,] = qstable(c(0.025, 0.975), alpha[ii], 0, exp(0.5 * xhat[tt,ii+1]), 0)  
  }
}
...
kk = 1
for (ii in 1:nAssets) {
  log_ret_label[ii] = paste(paste("log-returns (", asset_names[ii], ")", sep=""))
...
for (ii in 1:nAssets) {
  ...
  plot(grid, as.numeric(y$X0), col=plotColors[ii], type="l", main="", bty="n", xlab="date", 
       ylab=log_ret_label[ii], ylim=c(-25, 25), cex.lab=0.75, cex.axis=0.75, lwd=0.75)
  
  polygon(c(grid, rev(grid)), c(CI[ii,,1], rev(CI[ii,,2])), 
          border=NA, col=rgb(t(col2rgb(plotColors[ii])) / 256, alpha=0.15))
  
  par(new=TRUE)
  plot( grid, xhat[,ii+1], lwd=1, col="grey30", type="l", xaxt="n", yaxt="n", xlab="", 
        ylab="", bty="n", ylim=c(0,7))
  
  axis(4, cex.axis=0.75)
  mtext("smoothed log-volatility", side=4, line=3, cex=0.5)
  
  plot(yprob[,comp_idx[ii,1]], yprob[,comp_idx[ii,2]], pch=19, cex=0.5, xlab=par_name[ii,1], 
       ylab=par_name[ii,2], bty="n", col="darkgrey", cex.lab=0.75, cex.axis=0.75)
...
plot(grid, rowMeans(y[,-1]), pch=19, col="darkgrey", bty="n", ylab="log-returns", xlab="time", 
     cex=0.5, ylim=c(-45,25), xaxt="n", cex.lab=0.75, cex.axis=0.75)
...
lines(grid[seq(10,T)], rowMeans(VaRABC[,-1])[10:T], col=plotColors[4], lwd=1.5)

lines(grid[seq(10,T)], rowMeans(VaRSMC[,-1])[10:T], col=plotColors[5], lwd=1.5)
