...
class stPMH(object):
    ...
    # How many iterations should we run the sampler for and how long is the
    # burn-in
    nIter = None
    nBurnIn = None
    proposeRVs = False
    ...
    # PMH2: how many samples during the burn-in should we use to est the
    # Hessian
    ...
    # When should we print a progress report? Should prior warnings be written
    # to screen.
    ...
    def sampleProposal(self,):
        ...
        if (self.PMHtype == "PMH0"):
            ...
            self.thp[self.iter, :] = self.th[self.iter - 1, :] + 0.5 * self.stepSize**2 * np.dot(self.invHessian, self.gradient[
                                                                                                 self.iter - 1, :]) + np.random.multivariate_normal(np.zeros(self.nPars), self.stepSize**2 * self.invHessian)
            ...
            if (self.iter > self.memoryLength):
                self.thp[self.iter, :] = self.th[self.iter - self.memoryLength, :] + 0.5 * self.stepSize**2 * np.dot(self.gradient[self.iter - self.memoryLength, :], self.hessian[
                                                                                                                     self.iter - self.memoryLength, :, :]) + np.random.multivariate_normal(np.zeros(self.nPars), self.stepSize**2 * self.hessian[self.iter - self.memoryLength, :, :])
            else:
    ...
    def calculateAcceptanceProbability(self, sm,  thSys, ):
        ...
        # Run the smoother to get the ll-estimate, gradient and
        # hessian-estimate
        self.estimateLikelihoodGradientsHessians(sm, thSys)

        # Compute the part in the acceptance probability related to the
        ...
        if (self.PMHtype == "PMH0"):
            ...
            proposalP = lognormpdf(self.thp[self.iter, :], self.th[
                                   self.iter - 1, :] + 0.5 * self.stepSize**2 * self.gradient[self.iter - 1, :], self.stepSize**2 * np.eye(self.nPars))
            ...
            proposalP = lognormpdf(self.thp[self.iter, :], self.th[self.iter - 1, :] + 0.5 * self.stepSize**2 * np.dot(self.gradient[
                                   self.iter - 1, :],  np.linalg.pinv(self.hessian[self.iter - 1, :, :])), self.stepSize**2 * np.linalg.pinv(self.hessian[self.iter - 1, :, :]))
            proposal0 = lognormpdf(self.th[self.iter - 1, :],  self.thp[self.iter, :] + 0.5 * self.stepSize**2 * np.dot(self.gradientp[
                                   self.iter, :],   np.linalg.pinv(self.hessianp[self.iter, :, :])), self.stepSize**2 * np.linalg.pinv(self.hessianp[self.iter, :, :]))

        elif (self.PMHtype == "qPMH2"):

            if (self.iter > self.memoryLength):
                proposalP = lognormpdf(self.thp[self.iter, :],                   self.th[self.iter - self.memoryLength, :] + 0.5 * self.stepSize**2 * np.dot(self.gradient[
                                       self.iter - self.memoryLength, :],  self.hessian[self.iter - self.memoryLength, :, :]), self.stepSize**2 * self.hessian[self.iter - self.memoryLength, :, :])
                proposal0 = lognormpdf(self.th[self.iter - self.memoryLength, :],  self.thp[self.iter, :] + 0.5 * self.stepSize**2 * np.dot(self.gradientp[
                                       self.iter, :],                   self.hessianp[self.iter, :, :]),                   self.stepSize**2 * self.hessianp[self.iter, :, :])
            else:
                # Initial phase, use pPMH0
                proposalP = lognormpdf(self.thp[self.iter, :],   self.th[
                                       self.iter - 1, :], self.stepSize**2 * self.hessian[self.iter - 1, :, :])
                proposal0 = lognormpdf(self.th[self.iter - 1, :],  self.thp[
                                       self.iter, :], self.stepSize**2 * self.hessianp[self.iter, :, :])
        ...
        self.aprob[self.iter] = self.flag * np.exp(self.llp[self.iter, :] - self.ll[self.iter - 1, :] + proposal0 - proposalP + self.priorp[
                                                   self.iter, :] - self.prior[self.iter - 1, :] + self.Jp[self.iter, :] - self.J[self.iter - 1, :])

        # Store the proposal calculations
        self.proposalProb[self.iter] = proposal0
        self.proposalProbP[self.iter] = proposalP
        self.llDiff[self.iter] = self.llp[
            self.iter, :] - self.ll[self.iter - 1, :]
    ...
    def estimateLikelihoodGradientsHessians(self, sm, thSys,):
        ...
        # PMH1, only run the smoother and extract the likelihood estimate and
        # gradient
        if (self.PMHtypeN == 1):
            sm.smoother(thSys)
            self.gradientp[self.iter, :] = sm.gradient

        # PMH2, only run the smoother and extract the likelihood estimate and
        # gradient
    ...
    def checkHessian(self):
        ...
        if (~isPSD(self.hessianp[self.iter, :, :])):
            ...
            if (self.makeHessianPSDmethod == "regularise"):
                ...
                self.hessianp[self.iter, :, :] = self.hessianp[
                    self.iter, :, :] - 2.0 * mineigv * np.eye(self.nPars)
            ...
            # Replace the Hessian with the posterior covariance matrix after
            # burin
            if ((self.makeHessianPSDmethod == "hybrid") & (self.iter > self.nBurnIn)):
        ...
        if (self.iter > self.memoryLength):
            ...
            if (self.nHessianSamples[self.iter] > 2):
                ...
                for ii in range((self.thU).shape[0]):
                    ...
                    # Check if we have moved, otherwise to not add contribution
                    # to Hessian
                    if (np.sum(skk) != 0.0):
    ...
    def rejectParameters(self, thSys,):
        if ((self.PMHtype == "qPMH2") & (self.iter > self.memoryLength)):
            ...
            self.prior[self.iter, :] = self.prior[
                self.iter - self.memoryLength, :]
            self.gradient[self.iter, :] = self.gradient[
                self.iter - self.memoryLength, :]
            self.hessian[self.iter, :, :] = self.hessian[
                self.iter - self.memoryLength, :, :]
